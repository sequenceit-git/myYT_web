...
/**
 * Sends a 6-digit account registration OTP email using Resend
 */
...
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Account Verification Code - ytCash</title>
  <style>
    body {
      font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif;
      background-color: #f8fafc;
...
<body>
  <div class="container">
    <div class="header">
      <div class="logo-badge">YTCASH SECURITY</div>
      <div class="logo-sub">Official Account Registration Verification</div>
    </div>
    <div class="content">
      <div class="greeting">Welcome ${userName ? userName.trim() : 'to ytCash'}!</div>
      <div class="message">
        Thank you for creating an account on <strong>ytCash</strong>.
        Please use the 6-digit registration verification code below to activate your account.
      </div>
      
      <div class="code-wrapper">
        <div class="code-label">Account Verification OTP Code</div>
        <div class="code-value">${otpCode}</div>
      </div>

      <div class="info-box">
        <strong>Security Notice:</strong> This code is valid for <strong>15 minutes</strong>. Do not share this code with anyone.
      </div>
    ...
    <div class="footer">
      &copy; ${new Date().getFullYear()} ytCash Platform. All rights reserved.<br>
      Automated account security transmission. Please do not reply.
    </div>
  </div>
</body>
</html>
    ...
    const textContent = `Welcome to ytCash!\n\nYour 6-digit registration verification code is: ${otpCode}\n\nThis code expires in 15 minutes.\n\nytCash Security`;

    const resend = getResendClient();
    if (!resend) {
      console.warn(`[Resend Email] Cannot send OTP email to ${toEmail}: RESEND_API_KEY is not set.`);
      return {
        success: false,
        error: 'Email service is not configured. Please set RESEND_API_KEY.',
      };
    ...
    const result = await resend.emails.send({
      from: fromAddress,
      to: toEmail,
      subject: `Your ytCash Account Verification Code: ${otpCode}`,
      html: htmlContent,
      text: textContent,
    });
    ...
    if (result.error) {
      ...
      return {
        success: false,
        error: result.error.message || 'Failed to send OTP email',
      };
    }

    console.log(`[Resend Email] Registration OTP code sent successfully to ${toEmail}. Message ID: ${result.data?.id}`);
    return {
      success: true,
      id: result.data?.id,
    };
  } catch (err: any) {
    console.error(`[Resend Email] Exception while sending registration OTP to ${toEmail}:`, err);
    return {
      success: false,
      error: err.message || 'Error occurred while sending registration OTP email',
    };
  }
}
